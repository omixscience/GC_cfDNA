...
pdf('20240930_01_Differentiation_Low_ROC_EVD_v1.pdf', width = 8, height = 6)
tibble(x=perf_roc@x.values[[1]], y=perf_roc@y.values[[1]]) %>%
ggplot(aes(x=x, y=y)) + 
...
pdf('20240930_03_Differentiation_High_ROC_EVD_v1.pdf', width = 8, height = 6)
tibble(x=perf_roc@x.values[[1]], y=perf_roc@y.values[[1]]) %>%
ggplot(aes(x=x, y=y)) + 
  # geom_line(aes(color=cut(y, c(-1,0.5,0.7,0.9,1))), linewidth=2) +
  # geom_point(aes(color=cut(y, c(-1,0.5,0.7,0.9,1))), size = 3) +
...
pdf("20240930_04_Differentiation_Cancer_score_boxplot_EVD_v1.pdf", height=4.5, width=5)

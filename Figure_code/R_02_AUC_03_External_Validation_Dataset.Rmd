...
# 2. loading data
```{r}
# Clinical <- read.xlsx("/Users/xuewei/ZxProjects/20240401_GC_cfDNA/00_backup/GC_20240408_566_v3_xw.xlsx", sheet = 1, skipEmptyRows = FALSE)
...
write.xlsx(EVD_data, "GC_03_EVD_data_20240925.xlsx")
# write.table(EVD_data, file="GC_03_EVD_data_20240925.txt", sep = "\t", quote=F, row.names = F, col.names = T)
...
# 计算AUC和95%置信区间
roc_obj <- roc(EVD_data$actual, EVD_data$predicted_prob)
...
pdf('20240925_03_External_Validation_Dataset_ROC_v1.pdf', width = 8, height = 6)
tibble(x=perf_roc@x.values[[1]], y=perf_roc@y.values[[1]]) %>%
ggplot(aes(x=x, y=y)) + 
...
pdf('20240925_03_External_Validation_Dataset_Heatmap_v1.pdf', width = 8, height = 6)
ggplot(data = conf_matrix_df, aes(x = Prediction, y = Reference)) +
  ...
  scale_fill_gradient(low = "#FFFFFF", high = "#8e00ff") +
  theme_minimal() +
